...
<div class="container-fluid py-4">
                ...
                <div class="card my-4">
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                            <h6 class="text-white text-capitalize ps-3">Manage Uploaded NCP</h6>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive p-0">
                            ...
                            <table class="table align-items-center mb-0">
                                ...
                                <tbody>
                                    ...
                                    $query = "SELECT useruploads_ncp.*, ncp.*, useruploadfiles_ncp.*
                                    FROM useruploads_ncp
                                    INNER JOIN ncp ON useruploads_ncp.ncpid = ncp.ncpid
                                    INNER JOIN useruploadfiles_ncp ON useruploads_ncp.ncpuploadid = useruploadfiles_ncp.ncpuploadid WHERE (useruploads_ncp.status='1' OR useruploads_ncp.status='2')";
                                    ...
                                    ?>
                                            <tr class="">
                                                ...
                                                <td>
                                                    ...
                                                    <img src="<?= "../" . $row['ncppath'] . $row['ncpfile']; ?>" alt="" style="width:75px; height:75px;">
                                                    </h6>
                                                </td>
                                                <td>
                                                    <h6 class="mb-0 text-xxs text-dark text-wrap" style="width: 15rem;">
                                                        <a target="_blank" href="../useruploads/ncp/<?= $row['ncpfile'] ?>">
                                                            <?= $row['ncpfile'] ?>
                                                        </a>
                                                    </h6>
                                                </td>
                                                ...
                                                <td class="align-middle text-center text-sm align-items-center">
                                                    <a href="manage_uploaded_ncp.php?id=<?= $row['ncpuploadid']; ?>" class="btn btn-info btn-sm mb-0">View/Edit</a>
                                                    <form action="functions.php" method="POST" class="d-inline">
                                                        <button type="button" class="btn btn-danger btn-sm m-0" onclick="openDialog(<?= $row['ncpuploadid'] ?>)">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                    <?php }
        ...
        <dialog id="deletePrompt" class="dialog">
            <div class="dialog-content p-4">
                ...
                <div class="dialog-actions">
                    ...
                    <input type="hidden" name="ncpuploadid" id="ncpuploadid" value="">
                    <button type="submit" name="delete_uploaded_ncp" id="delete_uploaded_ncp" class="btn btn-danger btn-sm m-0">Delete</button>
    ...
    <script>
        ...
        const ncpuploadidInput = document.getElementById('ncpuploadid')
        const dialogSumbit = document.getElementById('delete_uploaded_ncp');
        const dialogCancel = document.getElementById('cancel-delete');

        function openDialog(id) {
            ncpuploadidInput.value = id;
            dialog.showModal();
        }
