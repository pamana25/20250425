...
switch ($method) {
    ...
    case 'GET':
        $userUploads = $LcpUserUpload->getAllUserUploads();
    ...
    case 'POST':
        ...
        $inputData = [
            'lssid' => isset($_POST['lssid']) && !empty($_POST['lssid']) ? $_POST['lssid'] : '',
        ...
        $fileDirectory = "../../useruploads/lss/";
        $filePathDB = "./useruploads/lss/";
        ...
        $userUploadId = $LcpUserUpload->createUserUpload($inputData);
        if (!$userUploadId) {
            echo json_encode([
                'status' => 'error',
        ...
        // Create file upload record and move file
        if ($LcpUserUploadFile->createLcpUserUploadFile($userUploadId, $newFileName, $filePathDB)) {
    ...
    case 'PUT':
        ...
        if ($LcpUserUpload->updateUserUpload($inputData)) {
    ...
    case 'DELETE':
        if (isset($_GET['lssuploadid'])) {
            ...
            if ($LcpUserUpload->deleteUserUpload($lssuploadid)) {
