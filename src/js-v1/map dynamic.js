// document.addEventListener("DOMContentLoaded", async () => {
  ...
  document.addEventListener('geojsonDataLoaded', () => {
    console.log(json_LatLonLocalSignificantSites_0)
  // Initialize map
  const map = L.map("map", {
    zoomControl: false,
    maxZoom: 28,
  ...
  const classificationIcons = {
    NationalCulturalTreasures: L.icon({
      iconUrl: "assets/icons/NationalCulturalTreasure.svg",
      iconSize: [50, 50],
    }),
    NationalHistoricalShrines: L.icon({
      iconUrl: "assets/icons/NationalHistoricalShrine.svg",
      iconSize: [50, 50],
    }),
    NationalHistoricalLandmarks: L.icon({
      iconUrl: "assets/icons/NationalHistoricalLandmark.svg",
      iconSize: [50, 50],
    }),
    NationalHistoricalMonuments: L.icon({
      iconUrl: "assets/icons/NationalHistoricalMonument.svg",
      iconSize: [50, 50],
    }),
    ClassifiedHistoricStructures: L.icon({
      iconUrl: "assets/icons/ClassifiedHistoricStructure_Fill-2.svg",
      iconSize: [45, 45],
    }),
    ImportantCulturalProperties: L.icon({
      iconUrl: "assets/icons/ImportantCulturalProperty.svg",
      iconSize: [50, 50],
    }),
    PresumedImportantCulturalProperties: L.icon({
      iconUrl: "assets/icons/PresumedImportantCulturalProperty_b-3.svg",
      iconSize: [50, 50],
    }),
    UNESCOWorldHeritageSites: L.icon({
      iconUrl: "assets/icons/UNESCOWorldHeritage-01.svg",
      iconSize: [50, 50],
    }),
    LocalCulturalProperties: L.icon({
      iconUrl: "assets/icons/RegisteredCulturalProperties_pin-01.svg",
      iconSize: [50, 50],
    }),
    RegisteredProperties: L.icon({
      iconUrl: "assets/icons/LocalCulturalProperties_pin-01.svg",
      iconSize: [50, 50],
    }),
  ...
  async function createPopupContent(feature, ncpID, userID) {
    const results = await fetchData(ncpID, "ncp");
  ...
  <table>
    <tr>
      <td><strong>${
        feature.properties["Property N"]
          ? autolinker.link(feature.properties["Property N"].toLocaleString())
          : ""
      }</strong></td>
    </tr>
    ${
...
`;
  ...
  async function createPopupContentLSS(feature, ossID, userID, classification) {
    const result = await fetchData(ossID, "lcp");
  ...
  <table>
    ...
    <tr>
      <td><br/><a href='galleries?area=${"test"}&areaid=${
...
`;
  ...
  if (localStorage.getItem("type") == "ncp") {
    ncpCheckbox.checked = true;
    map.addLayer(ncpLayer);
    localStorage.removeItem("type");
  } else if (localStorage.getItem("type") == "lcp") {
  ...
  const getLcpMapDetails = async () => {
    try {
      const response = await fetch("backend/api/lcp-map-details.php");
      const data = await response.json();
      return data;
    } catch (error) {
