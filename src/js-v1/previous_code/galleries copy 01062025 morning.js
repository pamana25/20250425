...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const displayNcpAndLcpData = async () => {
    ...
    try {
      ...
      const allData = [...lcp, ...ncp]; // Combine data
      ...
      const renderPage = () => {
        ...
        const paging = allData.slice(start, end);

        if (paging.length === 0) {
          if (currentPage === 1) {
            // No data for the first page
        ...
        if (paging.length >= 9) {
          ...
          console.log(paging.length, "paging");
        } else {
          loadMoreButton.classList.add("hidden");
        }
      ...
      // Call this after rendering each page to bind click events to new buttons
      attachPropertyListeners();
    } catch (error) {
  ...
  // Cache for all property data (HTML strings)
  ...
  if (inputSearch) {
    inputSearch.addEventListener("input", async () => {
      ...
      if (allPropertiesCache.length === 0) {
        const ncp = await displayNcpSelectedProperty(); // Fetch NCP data
        const lcp = await displayLcpSelectedProperty(); // Fetch LCP data
        allPropertiesCache = [...lcp, ...ncp]; // Cache all property data
      }
  ...
  function attachAreaButtonListeners() {
    ...
    btnArea.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        currentPage; // Reset the page for pagination
        displayNcpAndLcpData();
      });
    });
  }
    ...
    <div class="w-full lg:w-7/12">
     ${
      ...
      <div class="flex space-x-2 overflow-x-auto scrollbar-hide pb-2">
         ...
         : `<div class="w-full min-h-[600px] rounded-lg shadow-md text-white ${hasImageData ? "bg-white" : "bg-black"} my-auto content-center text-center p-5"><p>There are currently no images available. You can help the community by sharing photos of this property here.<a href="./upload-photos?${hasNcpId ? 'ncpid' : 'lcpid'}=${id}&areaname=${name}" class="ms-2 p-2 py-1 text-xs rounded bg-primary-600 text-white">Upload here</a></p></div>`
     }
    </div>
...
console.log(imageData[index].user_upload);
    ...
    `;
      ...
      document.getElementById("btnBack").addEventListener("click", () => {
        ...
        currentPage = 1; // Reset the page for pagination
        displayNcpAndLcpData();
      });
