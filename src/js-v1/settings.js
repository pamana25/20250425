...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const checkSubscribeStatus = async () => {
    try {
      const userSubsStatus = await getUser();
      const user = userSubsStatus.find(findUser => findUser.userid == userid);
  
      // Set the toggle state based on subscription status
      accountToggle.checked = user?.subscribe_email === 1;
  
      // Toggle event listener for account updates
      accountToggle.addEventListener("change", async () => {
        const toggleData = {
          toggleUpdate: true,
          toggleStatus: accountToggle.checked ? "on" : "off",
        };
        await toggleReceiveEmailAPI(toggleData);
      });
    } catch (error) {
  ...
  const changeInfoAPI = async (formData) => {
    try {
      const response = await fetch("backend/api/user.php", {
        method: "POST",
        headers: {
  ...
  changeInfoForm.addEventListener("submit", async (event) => {
    ...
    const formData = new FormData(changeInfoForm);
    formData.append("update_user", true);
    const registerData = Object.fromEntries(formData.entries());
    await changeInfoAPI(registerData);
  });
...
const toggleReceiveEmailAPI = async (toggleData) => {
  try {
    const response = await fetch(`backend/api/user.php`, {
      method: "POST",
      headers: {
