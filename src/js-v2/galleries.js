...
document.addEventListener("DOMContentLoaded", async function () {
        ...
        combinedProperties.map(async (item) => {
          ...
          if (item.ncpid) {
            ...
            if (item.uploadstatus === 1) {
              htmlString = `
                <article class="property-card cursor-pointer property-link rounded-lg shadow bg-white flex flex-col mx-3 mb-6">
                  <a data-id=${
                    item.ncpid
  ...
  const renderPage = (totalProperties) => {
    ...
    if (pagingData.length === 0) {
      ...
      // listOfProperties.innerHTML = ``;
      listOfProperties.innerHTML =
        areaIdParams && areaNameParams
          ? `<div class="no-data col-span-full">No properties enlisted. <br> <i class="text-gray-500">Help expand this page by nominating a heritage site. Reach out to us at community@pamana.org</i></div>`
          : `<div class="no-data">Select a city or municipality.</div>`;
      return;
    }
  ...
  <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div id="detailsPanel" class="bg-white rounded-lg shadow-md overflow-y-scroll lg:block fixed inset-y-0 right-0 w-full max-w-sm lg:static lg:max-w-none transform translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out lg:min-h-[600px] lg:h-[600px] z-[100]">
            ...
            <div class="border-t">
                <div class="flex">
                    ...
                    <button id="modelTab" class="flex-1 py-3 px-4 text-center font-medium text-gray-600 hover:bg-gray-50 ${
                      !model ? "hidden" : ""
                    }">3D Model</button>
                    ...
                    <div id="descriptionContent" class="space-y-4 p-6">
                        ...
                        ${
                          description.length > 0
                            ? description.split(/\s+/).length < 1000
                              ? `${description}<br><i>Help us improve this section by sharing information about this site. Reach out to us at community@pamana.org</i>`
                              : description
                            : `<i>Help us improve this section by sharing information about this site. Reach out to us at community@pamana.org</i>`
                        }
                        </div>
                    </div>
                     <div id="modelContent" class="space-y-4 ${
                       !model ? "hidden" : ""
                     } w-full]">
                        <div class="w-full flex justify-center items-center">
                          ${
                            model
                              ? ` <a id="modelLink" href="./model/${
                                  model.id
                                }" class="w-full p-3 hidden text-primary-500 text-center  rounded font-semibold">
                           <img src="assets/model-images/${
                             model && model.id
                           }.jpg" class="rounded h-64 mx-auto shadow mb-2"><img>
                           VIEW 3D MODEL
    ...
    if (hasImageData) {
      function updateCarousel(index) {
        ...
        console.log(
          imageData[index].user_upload.source &&
            imageData[index].user_upload.source_name
            ? imageData[index].user_upload.source
            : "#"
        );
        ...
        imageDescription.innerHTML = `
            <div class="flex gap-2 flex-wrap justify-center text-xs mb-2">
              <p class="text-white">${
                imageData[index].user_upload.description
              }</p>
        <a href="${
          imageData[index].user_upload.source &&
          imageData[index].user_upload.source_name
    ...
    const toggleDetailsPanelBackdrop = (show) => {
      const detailsPanelBackdrop = document.getElementById(
...
const pamanaTitle = (isDisplay, title = null) => {
  if (isDisplay) {
    ...
    document.title =
      "PAMANA | Preserving our cultural legacy for future generations.";
  }
};
const btnSearch = document.getElementById("btnSearch");
btnSearch.addEventListener("click", () => {
  document.querySelector(".properties-container").classList.toggle("flex");
  document.querySelector(".svgClose").classList.toggle("hidden");
  document.querySelector(".svgSearch").classList.toggle("hidden");
  document.getElementById("inputSearch").classList.toggle("hidden");
});
