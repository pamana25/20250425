...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const displaySelectedProperty = async () => {
    if (areaNameParams && areaIdParams) {
      ...
      findNcpProperty.sort((a, b) =>
        a.ncpname.localeCompare(b.ncpname)
      );
      findLcpProperty.sort((a, b) =>
        a.lssname.localeCompare(b.lssname)
      );

      // Combine and sort both arrays
      const combinedProperties = [...findNcpProperty, ...findLcpProperty].sort((a, b) => {
  ...
  const displayNcpAndLcpData = async () => {
    ...
    try {
      ...
      const renderPage = () => {
        ...
        const paging = properties.slice(start, end);
