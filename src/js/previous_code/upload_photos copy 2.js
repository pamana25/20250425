...
// console.log(propertiesUploadForm)
  ...
  //       ncp.map((ncp) => {
  //         if (option.getAttribute("value") == ncp.areaid) {
  //           optionValue.innerHTML += `<div id="ncpid" class='option property px-8 py-2 cursor-pointer bg-white hover:bg-gray-200' data-ncpid='${ncp.ncpid}'>${ncp.ncpname}</div>`;
  //         }
  //       });
  //     } else {
  //       lcp.map((lcp) => {
  //         if (option.getAttribute("value") == lcp.areaid) {
  //           optionValue.innerHTML += `<div id="lssid" class='option property px-8 py-2 cursor-pointer bg-white hover:bg-gray-200' data-lssid='${lcp.lssid}'>${lcp.lssname}</div>`;
  //         }
  //       });
  //     }
  //   });
  ...
  const categoryOptions = () => {
    ...
    options.forEach((option) => {
        ...
        if (!hasProperties) {
            optionValue.innerHTML = "<div class='bg-white no-properties text-gray-500 px-8 py-2'>No properties enlisted</div>";
        }
    });
};
  ...
  function handlePropertyClick(event) {
    if (
      ...
      selectBox.setAttribute("data-id", lssid); // Store ID in the data attribute
  ...
  //   if (fileLabel) {
  //     if (fileName) {
  //       fileLabel.textContent = fileName; // Set the label to the file name
  //     } else {
